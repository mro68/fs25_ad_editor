...
Seit den Rounding-Follow-ups CP-01 und CP-02 fuehrt der Route-Tool-Viewport-Vertrag zusaetzlich `RouteToolViewportData.prefers_generic_node_pick`, damit selektionsgetriebene Tools wie `RouteToolId::Rounding` Primarklicks wieder ueber den generischen Node-Pick und die bestehende Selection→Route-Tool-Synchronisierung routen koennen. Fuer selection-getriebene lokale Replace-Pfade fuehrt `RouteToolSelectionSeed` jetzt sowohl lineare Anschlussstrecken (`linear_stretches`) als auch eindeutige Anchor-Pfade (`anchor_paths`); `RouteToolEditPayload::{RoundingArc,RoundingQuadratic}` persistieren dazu ueberlebende Aussenanker beziehungsweise historische Anchor-Pfad-IDs, damit Preview, Execute und Recreate auch ueber Zwischen-Nodes denselben lokalen Arc-/Quadratic-Kontext rekonstruieren.
