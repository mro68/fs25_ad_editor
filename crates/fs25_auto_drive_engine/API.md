...
Seit den Rounding-Follow-ups CP-01 bis CP-04 fuehrt der Route-Tool-Viewport-Vertrag zusaetzlich `RouteToolViewportData.prefers_generic_node_pick`, damit selektionsgetriebene Tools wie `RouteToolId::Rounding` Primarklicks wieder ueber den generischen Node-Pick und die bestehende Selection→Route-Tool-Synchronisierung routen koennen. Fuer selection-getriebene lokale Replace-Pfade fuehrt `RouteToolSelectionSeed` weiterhin lineare Anschlussstrecken (`linear_stretches`) und eindeutige Anchor-Pfade (`anchor_paths`); das Arc-only-Verrunden nutzt davon den linearen Replace-Kontext und persistiert ueber `RouteToolEditPayload::RoundingArc` die beiden ueberlebenden Aussenanker, die urspruengliche Corner-Position, `max_angle_deg` sowie gemergte Durchfahrtsmetadaten. Preview, Execute und Recreate bleiben dadurch auch nach Entfernen des Corner-Nodes stabil.
...
`ColorPath` ist seit CP-05 als **Single-Step-Editor** modelliert: der UI-Contract kennt im `ColorPathPanelPhase`-Enum nur noch die drei kanonischen Phasen `Idle → Sampling → Editing`. Die kanonischen `ColorPathPanelAction`-Varianten sind `StartSampling`, `Compute`, `Accept` und `Reset`; `Compute` linearisiert intern die volle Stage-C-bis-F-Pipeline (`ColorPathTool::compute_to_editing`) aus `Sampling` direkt nach `Editing`. In `Editing` liegen Centerline-Preview, Junction-Drag und Stage-F unter einem Phasenlabel: Live-Slider rechnen Stage F im Tool nach, Junction-Drags bumpen die `EditableCenterlines.revision` und invalidieren Stage F lazy ueber den `PreparedSegmentsCacheKey`. `ColorPathPanelState` fuehrt dazu die kanonischen Flags `can_compute` und `can_accept`. Die Legacy-Phasen `Preview`/`CenterlinePreview`/`JunctionEdit`/`Finalize`, die Legacy-Actions `NextPhase`/`PrevPhase`/`ComputePreview`/`BackToSampling` und die Legacy-Flags `can_next`/`can_back` bleiben additiv als `#[deprecated]` erhalten (Engine setzt `can_next`/`can_back` konstant `false`, Legacy-Phasen werden im DTO auf `"editing"` gefaltet); die endgueltige Entfernung erfolgt in CP-11 (Sunset-Alias gemaess [Deprecation-Policy](../../docs/DEPRECATION_POLICY.md)). Das engine-interne Zwischenartefakt `EditableCenterlines` haelt Junction-Positionen und Segment-IDs zwischen Stage E und Stage F und ist nicht Teil der oeffentlichen Crate-API.
...
- Extern konsumierte Einstiegspunkte bleiben `app::{AppController, AppState, AppIntent, AppCommand}`, `core::RoadMap` und `xml::{parse_autodrive_config, write_autodrive_config}`.
- Kompat-/Legacy-Vertraege (z. B. ColorPath-Phasen) folgen der [Deprecation-Policy](../../docs/DEPRECATION_POLICY.md).

## Anti-Patterns

- Kein `egui`/`eframe`/Frontend-Toolkit-Import in dieser Crate (host-neutrale Fachlogik bleibt toolkit-frei).
- Keine neuen Crate-Root-Re-Exports ohne zugehoerigen API.md-Eintrag im jeweiligen Submodul.
