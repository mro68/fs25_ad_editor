...
/// Kontext fuer das Properties-Panel-Rendering.
///
/// Fasst alle Parameter von `render_properties_content()` zusammen,
/// um die Signatur kompakt zu halten.
pub struct PropertiesContext<'a> {
    /// Aktuelle RoadMap (nur fuer das Distanz-Panel benoetigt).
    pub road_map: Option<&'a RoadMap>,
    /// Aktuell selektierte Node-IDs.
    pub selected_node_ids: &'a IndexSet<u64>,
    /// Details des ersten selektierten Nodes (Einzelselektion).
    pub node_details: Option<&'a HostNodeDetails>,
    /// Verbindungs-Snapshot fuer Zwei-Node-Selektion.
    pub connection_pair: Option<&'a HostConnectionPairSnapshot>,
    /// Standard-Verbindungsrichtung.
    pub default_direction: ConnectionDirection,
    /// Standard-Verbindungspriorität.
    pub default_priority: ConnectionPriority,
    /// Schrittweite des Distanz-Scrollrads in Metern.
    pub distance_wheel_step_m: f32,
    /// Gruppen-Registry (optional).
    pub group_registry: Option<&'a GroupRegistry>,
    /// Tool-Edit-Store (optional).
    pub tool_edit_store: Option<&'a ToolEditStore>,
    /// Zustand des Distanzen-Panels (mutable).
    pub distance_state: &'a mut crate::app::state::DistanzenState,
}
...
pub fn render_properties_content(ui: &mut egui::Ui, ctx: PropertiesContext<'_>) -> Vec<AppIntent> {
...
fn render_single_node_info(
    ...
    let mut current_flag = NodeFlag::from(&details.flag);
    ui.horizontal(|ui| {
        ui.label("Flag:");
        egui::ComboBox::from_id_salt(("node_flag_editor", node_id))
            .selected_text(format!("{:?}", HostNodeFlag::from(&current_flag)))
            .show_ui(ui, |ui| {
    ...
    if HostNodeFlag::from(&current_flag) != details.flag {
...
fn render_connection_editor(
    ...
    ui.group(|ui| {
        ...
        let current_dir = ConnectionDirection::from(&conn.direction);
        ...
        if HostDefaultConnectionDirection::from(&selected_dir) != conn.direction {
        ...
        let current_prio = ConnectionPriority::from(&conn.priority);
        let mut selected_prio = current_prio;
        render_priority_icon_selector(ui, &mut selected_prio, &format!("{}_{}", start_id, end_id));

        if HostDefaultConnectionPriority::from(&selected_prio) != conn.priority {
