...
| `event_collection.rs` | Panels ueber `build_host_ui_snapshot(...)` und `build_host_chrome_snapshot(...)` lesen, Datei-/Pfad-Dialoge ueber `take_host_dialog_requests()` als `HostDialogRequest` drainen, `HostDialogResult` ueber `HostSessionAction::SubmitDialogResult` auf denselben Bridge-Dispatch-Pfad fuehren und Viewport-Input als gemischte Event-Pipeline (Intent + HostAction) buendeln |
...
6. Stateful Viewport-Host-Aktionen ueber `apply_host_action_with_viewport_input_state(...)` mit persistentem `HostViewportInputState` anwenden
7. Background-Sync aus den Assets des bereits fuer den Viewport aufgebauten RenderFrames ausfuehren und danach die Repaint-Entscheidung treffen
...
| `fn process_events(&mut self, ctx: &egui::Context, events: Vec<CollectedEvent>)` | Behandelt gemischte Events: lokale Intents, kanonische Host-Bridge-Seam fuer stabile Host-Aktionen und stateful Viewport-Host-Aktionen ueber `HostViewportInputState` |
| `fn collect_ui_events(&mut self, ctx: &egui::Context) -> Vec<CollectedEvent>` | Baut Panels ueber `build_host_ui_snapshot(...)` + `build_host_chrome_snapshot(...)`, drainet Dialog-Anforderungen ueber `take_host_dialog_requests(&controller, &mut state)` und sammelt Viewport-Gesten als `CollectedEvent::HostAction(SubmitViewportInput { ... })` oder lokale `CollectedEvent::Intent(...)` |
