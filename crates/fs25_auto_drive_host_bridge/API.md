...
- **ColorPath-Wizard (CP-04):** Der JSON-Transport des `color_path`-Panel-Zustands fuehrt seit dem Wizard-Umbau folgende Phase-Strings: `"idle"`, `"sampling"`, `"centerline_preview"`, `"junction_edit"`, `"finalize"`. Der Legacy-String `"preview"` bleibt additiv erhalten (wird von der Engine nicht mehr emittiert, `ColorPathPanelPhase::Preview` ist `#[deprecated]` und entfaellt in CP-11). Zusaetzlich liefert der Snapshot die Wizard-Flags `can_next`, `can_back` und `can_accept` neben dem bestehenden `can_compute`.
- **ColorPath-Wizard-Actions (CP-04):** `ColorPathPanelAction` kennt neben den bestehenden Eingangs-/Config-Actions die Wizard-Actions `NextPhase`, `PrevPhase` und `Accept` (Serde-Tags `"next_phase"`, `"prev_phase"`, `"accept"`). Die Legacy-Actions `ComputePreview` (`"compute_preview"`) und `BackToSampling` (`"back_to_sampling"`) bleiben als `#[deprecated]` Aliase erhalten und mappen in der Engine-Seam auf die neuen Wizard-Transitions; Hosts sollten aktiv auf die Wizard-Actions umziehen, da die Aliase in CP-11 entfernt werden.
