...
Dasselbe gilt fuer `session`: Die interne Implementierung ist in `session/{lifecycle,read_models,snapshots,context_menu,chrome_state,tests}.rs` sowie zusaetzlich in vier nach Verantwortlichkeit getrennte `impl HostBridgeSession`-Dateien aufgeteilt — `session_dispatch.rs` (Action-/Intent-Dispatch, Undo/Redo, Dialog-Drain), `session_snapshots.rs` (alle `build_*`/`snapshot*`-Methoden sowie `app_state()`), `session_chrome.rs` (Panel-/Dialog-/Floating-Menu-Seams) und `session_read_models.rs` (getypte und JSON-Read-Modelle) —, waehrend die oeffentliche Session-Surface (`HostBridgeSession` und zugehoerige Typen/Methoden) unveraendert bleibt.
...
| `pub fn update_overview_options_dialog(&mut self, snapshot: HostOverviewOptionsDialogSnapshot)` | Aktualisiert den host-lokalen Draft des Overview-Options-Dialogs (Layer-Visibility, ZIP-Pfad, Feldquellen). Muss vor `HostSessionAction::ConfirmOverviewOptions` aufgerufen werden, damit die Flutter-/Host-seitigen Aenderungen in den Engine-State uebernommen werden. Der Reconcile-Schritt synchronisiert diese Draft-Werte intern vor dem Confirm-Intent |
...
- **Reconcile-Semantik fuer Override-Dialoge:** `apply_action(HostSessionAction::ConfirmOverviewOptions)` führt intern einen Reconcile-Schritt durch, der den host-lokalen Draft-Zustand (gespeicherte Hintergrund-Layer, ZIP-Feldquellen) in den Engine-State synchronisiert, bevor der Confirm-Intent verarbeitet wird. Flutter- und Host-seitige Adapter müssen vor dem Confirm `update_overview_options_dialog(...)` aufrufen, um ihre lokalen Änderungen (Layer-Sichtbarkeit, ZIP-Pfad, Feldquellen) korrekt zu übernehmen.
...
- Der JSON-Transport der Route-Tool-Paneldaten (`host_ui_snapshot_json(...)`) enthaelt fuer `color_path` explizit `junction_radius`; Host-Adapter koennen den Radius fuer Junction-Trim damit ohne lokale Sonderlogik lesen und durchreichen.
- **ColorPath-Single-Step (CP-05):** Der JSON-Transport des `color_path`-Panel-Zustands fuehrt seit dem Single-Step-Umbau die drei kanonischen Phase-Strings `"idle"`, `"sampling"` und `"editing"`. Die Legacy-Phase-Strings `"preview"`, `"centerline_preview"`, `"junction_edit"` und `"finalize"` werden nicht mehr emittiert; deserialisierungstolerant bleiben sie als Aliase erhalten und faltet der DTO-Layer auf `"editing"` zurueck (`ColorPathPanelPhase::Preview`/`CenterlinePreview`/`JunctionEdit`/`Finalize` sind in der Engine `#[deprecated]` und entfallen in CP-11). Der Snapshot liefert die kanonischen Flags `can_compute` und `can_accept`; die Legacy-Flags `can_next` und `can_back` bleiben additiv im JSON, werden von der Engine aber konstant als `false` gesetzt.
- **ColorPath-Single-Step-Actions (CP-05):** `ColorPathPanelAction` kennt als kanonische Wizard-Tags `"start_sampling"`, `"compute"`, `"accept"` und `"reset"`. `Compute` (`"compute"`) loest aus `Sampling` heraus die volle Stage-C-bis-F-Pipeline aus und schaltet die Phase auf `Editing`. Die Legacy-Action-Tags `"compute_preview"`, `"back_to_sampling"`, `"next_phase"` und `"prev_phase"` bleiben deserialisierungstolerant als `#[deprecated]` Aliase erhalten; sie mappen Engine-seitig auf `Compute`/`Reset`. Hosts sollten aktiv auf die Single-Step-Actions umziehen, da die Aliase in CP-11 entfernt werden.
...
- Mutationen ausschliesslich ueber `HostBridgeSession::apply_action(HostSessionAction)`; `apply_intent(AppIntent)` nur als expliziter Uebergangs-Fallback fuer noch nicht kanonisierte Intents.
- Reads ueber die bridge-owned Snapshots (`snapshot()`, `dialog_snapshot()`, `editing_snapshot()`, `context_menu_snapshot()`, `build_host_ui_snapshot()`, `build_host_chrome_snapshot()`, `build_route_tool_viewport_snapshot()`, gekoppelter `build_render_frame(...)`).
- Neue Flutter-/FFI-Call-Sites verwenden die `Host*`-Typnamen; `Engine*`-Aliase sind nur fuer bestehenden Code gedacht (siehe [Deprecation-Policy](../../docs/DEPRECATION_POLICY.md)).
- Datei-/Pfad-Dialog-Lifecycle laeuft ueber `take_dialog_requests()` + `HostSessionAction::SubmitDialogResult`, nicht ueber eigene Dialog-Zustaende im Host.

## Anti-Patterns

- Keine eigene zweite Session- oder DTO-Familie neben `HostBridgeSession` fuer denselben Host aufbauen.
- Kein direkter `AppState`/`AppController`-Zugriff von aussen (die Bridge bleibt die einzige Seam); `app_state()` ist nur ein temporaerer read-only Escape-Hatch.
- Keine `egui`-, `eframe`- oder `wgpu`-Importe in diesem Crate (toolkit-frei, CI-gepueft ueber `check_layer_boundaries.sh` Regel 10/15).
- Keine neuen Perma-Compat-Aliase ohne Eintrag in der Deprecation-Policy anlegen.
