...
| `pub fn build_viewport_geometry_snapshot(state: &AppState, viewport_size: [f32; 2]) -> HostViewportGeometrySnapshot` | Baut einen vollstaendigen, serialisierbaren Geometry-Snapshot fuer FFI-/Polling-Hosts |
...
| `HostViewportGeometrySnapshot` | Vollstaendiger, serialisierbarer Viewport-Geometry-Snapshot fuer Nodes, Connections, Marker und Kamera-/Viewport-Metadaten |
...
| `pub fn chrome_state_mut(&mut self) -> &mut HostLocalDialogState` | Liefert den mutierbaren host-lokalen Chrome-/Dialogzustand und invalidiert den Session-Snapshot vorsorglich sofort |
...
| `pub fn dialog_ui_state_mut(&mut self) -> HostDialogUiState<'_>` | Liefert den schmalen Rust-Host-Seam fuer host-lokale Dialogzustands-Mutationen; snapshot-relevante Aenderungen werden ueber den Rueckgabe-Guard automatisch invalidiert |
...
| `pub fn build_viewport_geometry_snapshot(&self, viewport_size: [f32; 2]) -> HostViewportGeometrySnapshot` | Liefert einen vollstaendigen, serialisierbaren Geometry-Snapshot fuer Transport-Adapter |
...
- Die schmalen UI-Local-Seams (`HostPanelPropertiesState`, `HostDialogUiState`, `HostViewportInputContext`, `HostLocalDialogState`) sind bewusst Rust-Host-intern und nicht als serialisierbare FFI-DTO-Surface gedacht. Fuer serialisierbare Dialog-, Editing- und Kontextmenue-Daten stehen stattdessen `dialog_snapshot()` mit `HostDialogSnapshot`, `editing_snapshot()` mit `HostEditingSnapshot` sowie `context_menu_snapshot()` mit `HostContextMenuSnapshot` zur Verfuegung. `chrome_state_mut()` invalidiert den Session-Snapshot vorsorglich sofort und `dialog_ui_state_mut()` invalidiert snapshot-relevante Aenderungen automatisch. Die Seams `panel_properties_state_mut()` und `viewport_input_context_mut()` bleiben dagegen Snapshot-transparent; snapshot-relevante Escape-Hatch-Mutationen muessen dort explizit ueber `mark_snapshot_dirty()` invalidiert werden.
...
- Der serialisierbare Geometry-Snapshot bleibt bewusst read-only und vollstaendig: Nodes, Connections, Marker sowie Kamera-/Viewport-Metadaten werden transportiert. Der neue Write-Slice ist gezielt auf den stabilen Viewport-Input begrenzt und zieht keine Route-Tool-spezifischen oder Touch-Vertraege mit.
