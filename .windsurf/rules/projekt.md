...
- `build_viewport_geometry_snapshot(...)` ist ein read-only Volltransport (Nodes, Connections, Marker plus Kamera-/Viewport-Metadaten) und kein Minimal-Snapshot.
- Snapshot-Invalidierung fuer lokale Seams ist differenziert: `chrome_state_mut()` invalidiert sofort, `dialog_ui_state_mut()` invalidiert snapshot-relevante Aenderungen automatisch, `panel_properties_state_mut()`/`viewport_input_context_mut()` bleiben snapshot-transparent und nutzen bei Bedarf explizit `mark_snapshot_dirty()`.
