...
- Additiv dazu existiert `Texture-Registration-v4` als separater Vertrag mit gemeinsamer Capability-Negotiation, gemeinsamen Frame-Metadaten und gemeinsamem Lifecycle bei gleichzeitig getrennten Payload-Familien fuer Windows, Linux (DMA-BUF) und Android (AHardwareBuffer-ExportLease); die fruehere Android-Surface-Attach-Familie bleibt nur als Legacy-Kompatibilitaet im API-Slice erhalten.
- Der v4-Pfad bleibt ohne stillen Pixelbuffer- oder v3-Reinterpretations-Fallback explizit capability-gated: Windows und Linux melden weiterhin `NotYetImplemented` oder `Unsupported`, waehrend Android auf Android-Targets renderseitig `Supported` ueber `ExportLease` plus `AHardwareBuffer` ausweist.
- Echte externe Host-Registration bleibt zweistufig: Neben backend-nativem Export-/Attach-Code im Render-Stack braucht jeder Ziel-Host einen nativen Import-/Surface-Pfad fuer DXGI, DMA-BUF oder Android-AHardwareBuffer. Dieser zweite Host-Pfad liegt bewusst ausserhalb des aktuellen Rust-Repos.
...
- **Control-Plane:** `FlutterSessionHandle` (Arc<Mutex<HostBridgeSession>>) nutzt dieselbe kanonische Session-Surface wie C-ABI und egui. Kein zweiter Session-Vertrag. `flutter_api.rs` bleibt der sichere Rust-Helfer-Layer, waehrend `lib.rs` dieselben Seams kanonisch als direkte `fs25ad_flutter_session_*`-C-FFI-Surface fuer `dart:ffi`/`ffigen` exportiert.
...
- Die direkte `fs25ad_flutter_session_*`-C-FFI-Surface fuer `dart:ffi`/`ffigen` ist der einzige Rust-seitige Flutter-Vertrag; `flutter_rust_bridge` und Codegen-Glue sind entfernt
...
- Gespeicherte Overview-Layer bleiben ein reiner App-Layer-Lifecycle: Nach XML-Load oder manuellem Background-Load erkennt die App im selben Verzeichnis die kanonischen PNG-Dateien, verlangt `overview_terrain.png` als Pflichtbasis, komponiert die aktiven Overlays CPU-seitig zu genau einem `BackgroundMap` und faellt ohne Terrain-Basis auf Legacy-`overview.png` beziehungsweise `overview.jpg` zurueck
