...
pub fn compute_bypass_positions(
    ...
    let t_chain_end = (*dense.last().expect("invariant: dense ist nach Catmull-Rom und polyline_length-Guard nicht-leer") - dense[dense.len() - 2]).normalize_or_zero();
    ...
    let exit_pts = sample_bezier(
        bn,
        bn + t_at_bn * cp_dist,
        *chain_positions.last().expect("invariant: chain_positions ist nicht-leer – Load-Chain-Invariante") - t_chain_end * cp_dist,
        *chain_positions.last().expect("invariant: chain_positions ist nicht-leer – Load-Chain-Invariante"),
        half_spacing,
    );
