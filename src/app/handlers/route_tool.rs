...
fn execute_and_apply(state: &mut AppState) {
    ...
    if let Some(result) = result {
        ...
        // Gruppe in Registry speichern (immer)
...
pub fn init_chain_if_needed(state: &mut AppState) {
    ...
    let start_id = *ordered_ids.first().expect("invariant: ordered_ids ist nicht-leer nach positions.len()<2-Guard");
    let end_id = *ordered_ids.last().expect("invariant: ordered_ids ist nicht-leer nach positions.len()<2-Guard");
...
pub fn recreate(state: &mut AppState) {
    ...
    if let Some(result) = result {
        ...
        // Gruppe neu registrieren — der alte Record wurde durch invalidate_by_node_ids entfernt
        let record_id = state.group_registry.next_id();
        if let Some(tool) = state.editor.tool_manager.active_tool() {
