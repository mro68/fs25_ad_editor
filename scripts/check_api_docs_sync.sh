...
if [ "${#API_DOC_FILES[@]}" -eq 0 ]; then
  echo "[FAIL] Keine API.md-Dateien unter src/ gefunden"
  exit 1
fi
...
check_no_match 'FloatingMenuKind::Basics|FloatingMenuKind::SectionTools|route_tool_icon\(idx: usize\)' 'Floating-Menues und Route-Icons dokumentieren den gruppen-/id-basierten Vertrag' src/ui/API.md src/app/API.md docs/ARCHITECTURE_PLAN.md

# RouteOffsetTool fuehrt keinen ungenutzten Preview-Cache mehr.
check_no_match 'cached_preview' 'RouteOffsetTool-Doku enthaelt keinen toten Preview-Cache' src/app/tools/API.md

# Entfernte Bypass-Use-Case-Doku darf nicht mehr auftauchen.
check_no_match 'generate_bypass\(state\)|state\.ui\.bypass' 'Bypass laeuft ueber Route-Tool-Flow, nicht ueber eigenen Use-Case' src/app/use_cases/API.md
...
check_has_literal 'UiState::confirm_dissolve_group_id' 'src/ui/API.md dokumentiert den Group-Dissolve-Zustand' src/ui/API.md
check_has_literal 'show_confirm_dissolve_dialog' 'src/ui/API.md dokumentiert den Dissolve-Dialog' src/ui/API.md
check_has_literal 'AppIntent::DissolveGroupConfirmed { segment_id }' 'src/ui/API.md dokumentiert den bestaetigten Dissolve-Intent' src/ui/API.md
